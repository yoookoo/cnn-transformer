"""
Cross Layer Attention in Vision Transformer
"""
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 feedforward_type='linear', kernel_size=3, with_bn=True):
        super().__init__()
        ...
        if feedforward_type == 'linear':
            self.feedforward = Mlp(
                in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop
            )
        elif feedforward_type == 'conv':
            self.feedforward = LocalEnhancedFeedForward(
                in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        args:
            ...
            - feedforward_type (:obj:`str`): feedforward type in ['linear', 'conv']
            - local_size (:obj:`int`): kernel size in LocalEnhancedFeedForward
            - local_with_bn (:obj:`bool`): whether add bn in LocalEnhancedFeedForward
        """
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                feedforward_type=feedforward_type, kernel_size=local_size, with_bn=local_with_bn)
    ...
    def forward_features(self, x):
        ...
        # select the last cls_token
        return all_cls_token[:, -1]
...
@register_model
def clit_tiny_patch16_224(pretrained=False, **kwargs):
...
@register_model
def clit_base_patch16_224(pretrained=False, **kwargs):
    model = VisionTransformer(
...
@register_model
def clit_tiny_patch16_384(pretrained=False, **kwargs):
    model = VisionTransformer(
...
@register_model
def clit_small_patch16_384(pretrained=False, **kwargs):
    model = VisionTransformer(
...
@register_model
def clit_base_patch16_384(pretrained=False, **kwargs):
    model = VisionTransformer(
